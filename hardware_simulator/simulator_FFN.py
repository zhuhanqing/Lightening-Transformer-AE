...
class FFNPrediction():
        ...
        for _iter_N1 in range(math.ceil(N1 / (self.num_tiles * self.hw.core_height))):
            ...
            for _iter_N2 in range(iter_N2):
                for _iter_D in range(math.ceil(iter_D / self.num_pe_per_tile)):
                    ...
                    # if _iter_D == 0:
                    #     print(cylces_load_data_sram_rf)
    ...
    def get_latency_mzi(self, matrix_dim1, matrix_dim2, print_msg=False):
        """Get the latency of running attn on MZI"""
        latency_dict = {}
        latency_dict['total'] = [0, 0]
        latency_dict['comp'] = [0, 0]
        latency_dict['datamovement'] = [0, 0]
        ...
        self.weight_reuse_factor = N2 # fully weight-stationary
        for _iter_N1 in range(math.ceil(iter_N1 / self.num_tiles)):
            # pre-load a chunk of data, the other loading would be hide in computation cylces
            num_bytes_dram_sram = (
                iter_D * (self.hw.num_mzis + self.hw.mzi_sigma_dim) + D * self.weight_reuse_factor)
            # print(num_bytes_dram_sram)
            cycles_preload_data_dram_sram += self.SRAM.preload_DRAM_SRAM(
                num_bytes_dram_sram, bits=self.in_bit, bandwidth_ratio=1/self.num_tiles)
            
            num_bytes_GB_sram = (iter_D * (self.hw.num_mzis + self.hw.mzi_sigma_dim) + D * self.weight_reuse_factor) * self.num_tiles
            cycles_preload_data_GB_SRAM += self.SRAM.load_GB_SRAM(num_bytes_GB_sram, bits=self.in_bit, bandwidth_ratio=1)
            
            for _iter_D in range(math.ceil(iter_D // self.num_pe_per_tile)):
                for _iter_N2 in range(iter_N2 // self.weight_reuse_factor):
                    num_bytes_sram_rf = max(
                        (self.hw.num_mzis + self.hw.mzi_sigma_dim), self.hw.core_width * self.weight_reuse_factor)
                    cylces_load_data_sram_rf += self.SRAM.load_SRAM_RF(
                        num_bytes_sram_rf, bits=self.in_bit, bandwidth_ratio=1/self.num_pe_per_tile)
    ...
    def get_latency_mrr_bank(self, matrix_dim1, matrix_dim2, print_msg=False):
        """Get the latency of running attn on MRR bank"""
        latency_dict = {}
        latency_dict['total'] = [0, 0]
        latency_dict['comp'] = [0, 0]
        latency_dict['datamovement'] = [0, 0]
        ...
        cylces_load_data_sram_rf = 0
        cycles_preload_data_GB_SRAM = 0
        for _iter_N1 in range(iter_N1 // self.num_tiles):
            # pre-load a chunk of data, the other loading would be hide in computation cylces
            num_bytes_dram_sram = (
                self.hw.core_height * D * full_range_support_factor_w)
            cycles_preload_data_dram_sram += self.SRAM.preload_DRAM_SRAM(
                num_bytes_dram_sram, bits=self.in_bit, bandwidth_ratio=1/self.num_tiles)
            num_bytes_GB_sram = (self.hw.core_height * D * full_range_support_factor_w + self.weight_reuse_factor * D * full_range_support_factor_x) * self.num_tiles
            cycles_preload_data_GB_SRAM += self.SRAM.load_GB_SRAM(num_bytes_GB_sram, bits=self.in_bit, bandwidth_ratio=1)

            for _i in range(full_range_support_factor_w):
                for _iter_D in range(iter_D // self.num_pe_per_tile):
                    for _iter_N2 in range(iter_N2 // self.weight_reuse_factor):
                        # load data from sram to rf
                        # have sram Q and sram K
                        num_bytes_sram_rf = max(
                            self.hw.core_height * self.hw.core_width, self.hw.core_width * self.weight_reuse_factor * full_range_support_factor_x)
                        # bandwidth is 1 / self.num_pe_per_tile since all pes in the same tile share 1 sram
                        cylces_load_data_sram_rf += self.SRAM.load_SRAM_RF(
                            num_bytes_sram_rf, bits=self.in_bit, bandwidth_ratio=1/self.num_pe_per_tile)
    ...
    def get_energy_crossbar(self, matrix_dim1, matrix_dim2, print_msg):
        ...
        # print(f'ADC energy -> {A2D_energy}: numbers -> {ps_size}')
        # print(f'time_accum -> {time_accum_factor}: numbers -> {math.ceil(iter_D / time_accum_factor)} -> {math.ceil(math.ceil(iter_D / time_accum_factor) / self.num_pe_per_tile)}')
        ...
        # we need to load and unload output
    ...
    def get_energy_mrrbank(self, matrix_dim1, matrix_dim2, print_msg=False):
        ...
        energy_comp = energy_comp_laser + energy_comp_D2A + energy_comp_TX + energy_comp_comp + energy_comp_output + energy_comp_adder
