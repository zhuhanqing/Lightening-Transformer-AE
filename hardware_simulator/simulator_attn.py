...
class attnPrediction():
    ...
    def __init__(self, op_info, configs=None) -> None:
        ...
        # self.num_pe_per_tile = hardware_info["num_pe_per_tile"]
    ...
    def get_latency_crossbar(self, matrix_dim1, matrix_dim2, print_msg=False):
        """Get the latency of running attn on dota array, the memory part is dedicated designed to avoid being memory-bounded"""
        # latency, cycles
        latency_dict = {}
    ...
    def get_energy_crossbar(self, matrix_dim1, matrix_dim2, print_msg=False):
        ...
        if self.adc_share_flag:
            ...
            energy_comp_adder = h * Adder_energy * \
                (ps_size) * 1e-9
        # print(f'ADC energy -> {A2D_energy}: numbers -> {ps_size * h}')
    ...
    def get_energy_mrrbank(self, matrix_dim1, matrix_dim2, print_msg=False):
        ...
        ## DAC
        energy_comp_D2A_1= D2A_energy * h * \
        ...
        energy_comp_comp += comp_energy_dynamic * h * \
            (num_computation // self.weight_reuse_factor) * \
            1e-9 * full_range_support_factor_w
        ...
        # NoC send to adder
        energy_dm_RF += h * self.num_byte * (N1 * N2 * iter_D * self.full_range_support_factor) * self.data_movement_NoC  * self.num_byte
